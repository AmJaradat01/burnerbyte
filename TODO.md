...
- [x] **SSO/OIDC** — `GET /auth/sso/:provider`, `GET /auth/sso/:provider/callback`; integrate `coreos/go-oidc/v3`; auto-verify SSO users; enforce_sso org setting
...
- [x] **Swagger/OpenAPI** — Add `swaggo/swag` annotations, wire `GET /api/v1/docs` and `GET /api/v1/docs/openapi.json`
...
- [x] **Notifications WebSocket** — `WS /api/v1/ws/notifications`; user-level push notifications channel
...
- [x] **Reconciler worker** — Redis ↔ PostgreSQL inbox reconciliation; config fields exist (`ReconcilerInterval`) but no implementation
...
### Missing Pages
- [x] **Profile page** (`/profile`) — User settings: display name, avatar, change password
- [x] **Profile sessions** (`/profile/sessions`) — List active sessions, revoke individual/all
- [x] **Profile delete** (`/profile/delete`) — Account deletion with password confirmation
- [x] **Dashboard** (`/dashboard`) — Org overview with stats, recent activity, org switcher
- [x] **Domain detail** (`/domains/:domainId`) — DNS verification status, assigned teams, per-domain settings
- [x] **Email detail** (`/email/:emailId`) — Full email view with attachment download links
- [x] **Landing page** (`/`) — Marketing/features page (currently minimal or redirect)

### Missing Page Sections
- [x] **Team settings tab** — Per-team settings page within `/teams`
- [x] **Team analytics tab** — Per-team analytics within `/teams` (API exists, no dedicated UI section)

### UX Enhancements
- [x] **Dark/light mode** — Theme toggle with system preference default; persist choice
- [x] **Command palette** — Cmd+K quick navigation across pages
- [x] **Breadcrumbs** — Org → Team → Domain → Inbox contextual navigation
- [x] **Skeleton loaders** — Loading states on all data-fetching pages
- [x] **Empty states** — Illustrations with clear CTAs when lists are empty
- [x] **Responsive/mobile** — Mobile-friendly layout for quick inbox checks

---

## Infrastructure
- [x] **SMTPD Dockerfile** — Exists but binary is a stub; update after SMTPD is wired
- [x] **Docker Compose** — Add SMTPD service entry once binary works
- [x] **CI/CD pipeline** — `.gitlab-ci.yml` for lint, build, test, deploy stages
