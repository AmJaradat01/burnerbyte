...
Claim mappings are configured per provider via the `claim_mappings` field. The `custom_claims` field on the provider specifies which additional claims to request from the IdP.
