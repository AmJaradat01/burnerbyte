...
If database providers are also configured, both the file-based provider and database providers are loaded.
...
8. Server redirects to the frontend login page with a one-time `sso_code` (60s TTL), which the frontend exchanges for tokens via `POST /auth/sso/exchange` — the web UI requests the refresh token as an httpOnly cookie

The same flow supports **account linking** — authenticated users can link additional SSO identities by initiating the flow with `?intent=link`. Linked identities are listed at `GET /auth/me/sso` and can be removed with `DELETE /auth/me/sso/{provider}`.

## Auto-Provisioning

When `auto_provision` is enabled on a provider, new users are created automatically on first SSO login. The provider configuration controls the defaults:
...
If domain mappings exist for the user's email domain, those take precedence over the default team and role settings.
...
Claim mappings are configured per provider via the `claim_mappings` field. The `custom_claims` field on the provider specifies which additional claims to request from the IdP. Note that `org_role` must be a valid org role (`owner`, `admin`, `member`) and `team_role` must be a valid team role (`lead` or `member`).
...
Platform admins can set the lock via `PATCH /admin/users/:id` with the `auth_method_lock` field.
...
Migrating to SSO clears the user's password hash and sets the auth method lock to `sso`. Migrating to password requires a `new_password` and sets the lock to `password`.
...
| `POST` | `/admin/sso/providers` | Create a new provider |
| `GET` | `/admin/sso/providers/:pid` | Get a single provider |
| `PUT` | `/admin/sso/providers/:pid` | Update a provider |
| `DELETE` | `/admin/sso/providers/:pid` | Delete a provider |
| `POST` | `/admin/sso/test` | Test provider connectivity |

### Domain Mappings

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/admin/sso/providers/:pid/domain-mappings` | Create a domain mapping |
| `GET` | `/admin/sso/providers/:pid/domain-mappings` | List mappings for a provider |
| `PUT` | `/admin/sso/providers/:pid/domain-mappings/:mid` | Update a domain mapping |
| `DELETE` | `/admin/sso/providers/:pid/domain-mappings/:mid` | Delete a domain mapping |
| `POST` | `/admin/sso/domain-mappings/preview` | Preview mapping results (dry-run) |

### Auth Migration

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/admin/users/:id/migrate-auth` | Migrate user auth method (`target`: `sso` or `password`) |
