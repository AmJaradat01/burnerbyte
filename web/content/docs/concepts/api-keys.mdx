...
API keys are `bb_` followed by 64 hex characters (32 bytes of CSPRNG output). The raw key is never stored: the database keeps a SHA-256 hash of the full key plus an 11-character prefix (`bb_` and the first 8 hex chars) for display. The raw key is never stored; the database keeps a SHA-256 hash of the full key (used for verification) plus a short key prefix (used for display and identification).
