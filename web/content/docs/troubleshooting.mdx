...
6. **Redis running?** — The SMTP router uses Redis for fast inbox lookups. Check `GET /readyz`
...
3. **JWT valid?** — WebSocket connections authenticate via `?token=<jwt>` query parameter
...
INSERT INTO setup_state (id, completed, completed_at)
...
2. **MinIO running?** — Check `GET /admin/health` for MinIO status
3. **Bucket exists?** — The bucket specified in `BB_MINIO_BUCKET` must exist. MinIO auto-creates it on first use if permissions allow
...
1. **Check health endpoint** — `GET /admin/health` shows service latencies
2. **Database indexes** — Ensure all migrations are applied (52+ indexes)
...
- **Health Check:** `GET /admin/health` — Service status and latencies
