...
| Key | Env Var | Default | Description |
|---|---|---|---|
| `encryption.key` | `BB_ENCRYPTION_KEY` | — | **Required.** 32-byte hex-encoded key for encrypting sensitive data (SSO secrets, etc.) |
...
| `rate_limit.trusted_proxies` | `BB_RATE_LIMIT_TRUSTED_PROXIES` | — | Comma-separated proxy IPs trusted for client-IP resolution |
...
| `password_policy.bcrypt_cost` | `BB_PASSWORD_POLICY_BCRYPT_COST` | — | bcrypt hashing cost (higher is slower and stronger) |
...
| `email_verification.ttl` | `BB_EMAIL_VERIFICATION_TTL` | — | Verification link lifetime |
