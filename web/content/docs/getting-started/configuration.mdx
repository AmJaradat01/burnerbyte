...
this reference is registered and covered by `TestEnvOverridesReachNestedKeys`; if
you add a new config key, register it with `SetDefault` (or `BindEnv`) or its
environment override will be silently discarded.
</Callout>
