...
The `admin_stats` job short-circuits when no admin client is connected — it skips the stats query rather than computing a snapshot nobody will receive.

Every API process starts all seven workers. There is no leader election and no flag to run an API instance without them, so a second replica duplicates every job — see [Production Deployment](/docs/self-hosting/production).
