...
The pending token expires after 5 minutes. SSO logins are not affected — they continue to auto-revoke the oldest session when the limit is reached.
...
| Auth | `POST /auth/register`, `POST /auth/login`, `POST /auth/login/resolve`, `POST /auth/refresh`, `GET /auth/me`, `PATCH /auth/me`, `PUT /auth/me/password`, `DELETE /auth/me`, `POST /auth/forgot-password`, `POST /auth/reset-password`, `GET /auth/verify-email/:token`, `GET /auth/sso-status`, `GET /auth/datetime-settings` |
...
| Roles | `GET /roles` |
| WebSocket | `GET /ws/inboxes/:id`, `GET /ws/notifications`, `GET /ws/admin-stats` |
| Docs | `GET /docs`, `GET /docs/openapi.json` |
| Health | `GET /healthz`, `GET /readyz`, `GET /metrics` (root level, not under `/api/v1`) |

## WebSocket Endpoints

All WebSocket endpoints authenticate via a `token` query parameter containing a valid JWT.

### Inbox Stream

```
GET /ws/inboxes/{inboxId}?token=<jwt>
```
