...
| Auth | `POST /auth/register`, `POST /auth/login`, `POST /auth/login/resolve`, `POST /auth/refresh`, `GET /auth/me`, `PATCH /auth/me`, `PUT /auth/me/password`, `DELETE /auth/me`, `POST /auth/forgot-password`, `POST /auth/reset-password`, `GET /auth/verify-email/:token`, `GET /auth/sso-status`, `GET /auth/datetime-settings`, `GET /auth/login/pending-sessions`, `POST /auth/sso/exchange` |
