...
// ListPlatform returns paginated platform-level audit events — those with no
// owning organization (org_id IS NULL), such as registration, login, password
// reset, account deletion, and session revocation. System-admin only.
func (r *AuditRepo) ListPlatform(ctx context.Context, filter domain.AuditFilter, page, perPage int) ([]domain.AuditEntry, int, error) {
	query := `SELECT a.id, a.actor_id, COALESCE(u.email,''), a.action, a.resource_type, a.resource_id, a.metadata, host(a.ip_address), a.created_at, COALESCE(a.user_agent,''), COALESCE(a.resource_name,''), COALESCE(a.actor_display_name,''), COALESCE(a.severity,'info'), COALESCE(a.category,'') FROM audit_logs a LEFT JOIN users u ON a.actor_id = u.id WHERE a.org_id IS NULL`
	countQuery := `SELECT COUNT(*) FROM audit_logs a WHERE a.org_id IS NULL`
	var args []any
	idx := 1
	...
	for rows.Next() {
		var e domain.AuditEntry
		var metadata []byte
		if err := rows.Scan(&e.ID, &e.ActorID, &e.ActorEmail, &e.Action, &e.ResourceType, &e.ResourceID, &metadata, &e.IPAddress, &e.CreatedAt, &e.UserAgent, &e.ResourceName, &e.ActorDisplayName, &e.Severity, &e.Category); err != nil {
			return nil, 0, err
		}
		_ = json.Unmarshal(metadata, &e.Metadata)
		entries = append(entries, e)
